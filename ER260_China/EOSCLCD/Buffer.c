...
{
	BCD DiscAmt; //jdb2018-07-03 保存ApplVar.DiscAmt
	...
	if (ApplVar.BufCC || ApplVar.FTrvoid)   /* reset CC bit ? */
	{
			...
			else
			{
				...
				if (rec->Key.Code > PLU1 && !discount) /* plu ? */
		    	{
					...
					CheckMixMatch(2);
						...
						//jdb2018-07-03 有特殊出价，ApplVar.Amt 需减去 ApplVar.DiscAmt
						DiscAmt = ApplVar.DiscAmt;
						ApplVar.DiscAmt.Sign = ApplVar.Amt.Sign;
						Subtract(&ApplVar.Amt, &ApplVar.DiscAmt);
						ApplVar.DiscAmt = DiscAmt;

						//jdb2018-07-03 加入折扣报表
						DiscAmt = ApplVar.Amt;
		...
		AddGroupTotal();
		if(TESTBIT(ApplVar.SCPWDDiscFlag,BIT0)){
			//jdb2018-08-09 销售时SC PWD折扣没有加入单品，部类报表,取消时也不用计算
		}else
			AddDeptTotal();
		if (rec->Key.Code > PLU1) /* plu ? */
		{
...
void ProBuf()
{
	...
	UnLong TempL ;
	BCD DiscAmt, Amt; //jdb2018-07-03 保存ApplVar.DiscAmt, ApplVar.Amt
	lastrefund = 0;
		...
		else
		{
			...
			if (ApplVar.RGRec.Key.Code < CORREC + 100)  /* correction */
			{
				...
				if (ApplVar.BufCC || ApplVar.FTrvoid)    /* reset CC bit ? */
				{
					...
					{ //jdb2018-07-03 ApplVar.Amt 减去特殊出价
						DiscAmt = ApplVar.DiscAmt;
						SETBIT(ApplVar.FZV, BIT5); //jdb2018-07-03 不加入折扣报表标志
						CheckMixMatch(0);
						RESETBIT(ApplVar.FZV, BIT5);
						ApplVar.DiscAmt.Sign = ApplVar.Amt.Sign;
						Subtract(&ApplVar.Amt, &ApplVar.DiscAmt);
						ApplVar.DiscAmt = DiscAmt;
	    	...
	    	else if (ApplVar.RGRec.Key.Code < DISC + 100)  /* discount */
			{
				...
				if (ApplVar.BufCC || ApplVar.FTrvoid)   /* reset CC bit ? */
				{
					...
					{
						if(ApplVar.RGRec.Key.Code == DISC_SCSUB_PERDEC || //jdb2018-08-09标记为SC PWD折扣
							ApplVar.RGRec.Key.Code == DISC_PWDSUB_PERDEC)
						{
							SETBIT(ApplVar.SCPWDDiscFlag,BIT0);
						}
						...
						if(ApplVar.RGRec.Key.Code == DISC_SCSUB_PERDEC || //jdb2018-08-09SC PWD折扣标记清零
							ApplVar.RGRec.Key.Code == DISC_PWDSUB_PERDEC)
						{
							RESETBIT(ApplVar.SCPWDDiscFlag,BIT0);
							CalculateScPwdDisc(1);//jdb2018-08-09 SC PWD折扣加入了PLU DEPT报表，取消时报表计算PLU DEPT
						}
					}
				}
    		}
...
//jdb2018-04-11 计算免税额度
BCD CalculateNetTax(struct TRANSRECORD  MyRGRec)
{
	BYTE i, t;
	WORD SaveDeptNumber, PluNumber;
	struct DEPTRECORD   SaveDept;
	BYTE Tax = 0x0;
	BYTE SaveTaxNumber;
	struct TAXRECORD    SaveTax;
	BCD Qty, Amt, DiscAmt;
	BCD matchqty, soldqty, freeqty, offamt;//jdb2018-08-09 计算特殊出价
	...
	if(MyRGRec.Key.Code < PLU1){
		SaveDeptNumber = ApplVar.DeptNumber;
		ApplVar.DeptNumber = MyRGRec.Key.Code - DEPT - 1;
		SaveDept = ApplVar.Dept;
		ReadDept();
		Tax = ApplVar.Dept.Tax;
		ApplVar.DeptNumber = SaveDeptNumber;
		ApplVar.Dept = SaveDept;
	}else{
		PluNumber = MyRGRec.Key.Code - PLU1 - 1;
		SaveDeptNumber = ApplVar.DeptNumber;
		#if 1 
		ApplVar.PluNumber = PluNumber;
		ReadPlu();
		ApplVar.DeptNumber = ApplVar.Plu.Dept;
		if (TESTBIT(ApplVar.MyFlags,PLUPACK))//jdb2018-08-09 特殊打包出价情况
		{
			matchqty = ZERO;
		...
		else if(TESTBIT(ApplVar.MyFlags,PLUDISC))//jdb2018-08-09 特殊出价折扣出价情况
		{
			offamt = ZERO;
            memcpy(offamt.Value, ApplVar.Plu.Price[1], ApplVar.AP.Plu.PriceSize);
            offamt.Sign = 4;
			soldqty = ZERO;
			soldqty = MyRGRec.DiscAmt;
			Multiply(&offamt, &soldqty);
            RoundBcd(&offamt,0);
            offamt.Sign ^= 0x80;
		}
		#else
		ApplVar.DeptNumber = ReadPluDept(PluNumber);
		#endif
		SaveDept = ApplVar.Dept;
		ReadDept();
		Tax = ApplVar.Dept.Tax;
	...
	for (i = 0; i < 8; i++)
    {
	    if (Tax & t)
	    {
			...
			ReadTax();
			Qty = ZERO;
			memcpy(Qty.Value, ApplVar.Tax.Rate, sizeof(ApplVar.Tax.Rate));
			Amt = ZERO;
			Amt = MyRGRec.Amt;
			...
			{
				//jdb2018-04-10
				Multiply(&Amt, &MyRGRec.Qty);
				RoundBcd(&Amt, 0);
				AmtRound(1, &Amt);
				RESETBIT(Amt.Sign, BIT7);

				//jdb2018-07-02 取消，退货操作，先减去特殊出价，再计算免税额
					#if 1
					Add(&Amt, &offamt);//jdb2018-08-09 减去特殊出价
					#else
					DiscAmt = ApplVar.DiscAmt;
					SETBIT(ApplVar.FZV, BIT5); //jdb2018-07-03 不加入折扣报表标志
					ApplVar.MixQty = ZERO;
					CheckMixMatch(0);
					RESETBIT(ApplVar.FZV, BIT5);
					ApplVar.DiscAmt.Sign = Amt.Sign;
					Subtract(&Amt, &ApplVar.DiscAmt);
					ApplVar.DiscAmt = DiscAmt;
					#endif

				DiscAmt = Amt;	/* contains NET */
				Add(&Qty, (BCD*)&THOUSAND10);
				Qty.Sign = 0x04;
				Divide(&DiscAmt, &Qty);
				RoundTaxable(&DiscAmt);
				Subtract(&Amt, &DiscAmt);
				//Amt.Sign |= 0x80;
			}
...
//jdb2018-04-11 5 取消交易
void ProBuf2(BYTE type)
{
	BYTE i;
	BCD Val1;
	BCD sVal1, sVal2, sVal3, sVal4;
	struct TRANSRECORD  TmpRGRec;
	
	for(i = 0; i < ApplVar.NoVatRGItems.RGNumber; i++){
		if(type == 5){
			Val1 = ZERO;
			TmpRGRec = ApplVar.NoVatRGItems.RGBuf[i];
			Val1 = CalculateNetTax(TmpRGRec);
	...
	if(type == 5){
		ApplVar.NoVatRGItems.RGNumber = 0;
		return;
	}
}

//jdb2018-04-09
//0 增加 1 赋值 --- 2 比较数量 3 计税 4 矫正
//5查询是否有记录 
int StoreInBuffer2(BYTE type, struct TRANSRECORD  MyRGRec)
{
	BYTE i, t;
	short j;
	BCD TmpQty, Qty, Amt, DiscAmt;
	WORD SaveDeptNumber, PluNumber;
	struct DEPTRECORD   SaveDept;
	BYTE Tax = 0x0;
	BYTE SaveTaxNumber;
	...
	//jdb2018-04-10 记录最后一次销售免税额
	if(type == 0){
	#if 0
	ApplVar.LastNetVatAmt = CalculateNetTax(MyRGRec);
	#else
	if(MyRGRec.Key.Code < PLU1){
		SaveDeptNumber = ApplVar.DeptNumber;
		ApplVar.DeptNumber = MyRGRec.Key.Code - DEPT - 1;
	...
	for(i = 0; i < ApplVar.NoVatRGItems.RGNumber; i++){
			if (ApplVar.NoVatRGItems.RGBuf[i].Key.Code == MyRGRec.Key.Code){
				if (CompareBCD(&ApplVar.NoVatRGItems.RGBuf[i].Amt, &MyRGRec.Amt))
						continue;/* 价格不相同,不能合并 */
				if (!CheckNotZero(&ApplVar.NoVatRGItems.RGBuf[i].Qty))
						continue;   /* 数量为0,不能合并 */
				if(type == 5)//jdb2018-08-09 有记录返回序号
					return i;
				j = CompareBCD(&ApplVar.NoVatRGItems.RGBuf[i].Qty, &MyRGRec.Qty);
				if(type == 4){
					if(j > 0){
						ApplVar.NoVatRGItems.RGBuf[i].Qty = MyRGRec.Qty;
					}
					return 0;
				}
				if(type == 2){
					j = CompareBCD(&MyRGRec.Qty, &ApplVar.NoVatRGItems.RGBuf[i].Qty);
					if(j > 0){
						return 1;
					}else{
						return -1;
				...
				if (type == 3){  /* correction */
					if(j < 0)
						continue;
					Add(&TmpQty, &ApplVar.NoVatRGItems.RGBuf[i].Qty);
					if(ApplVar.NoVatRGItems.RGBuf[i].Key.Code < PLU1){
						SaveDeptNumber = ApplVar.DeptNumber;
						ApplVar.DeptNumber = ApplVar.NoVatRGItems.RGBuf[i].Key.Code - DEPT - 1;
						SaveDept = ApplVar.Dept;
						ReadDept();
						Tax = ApplVar.Dept.Tax;
						ApplVar.DeptNumber = SaveDeptNumber;
						...
						PluNumber = ApplVar.NoVatRGItems.RGBuf[i].Key.Code - PLU1 - 1;
						SaveDeptNumber = ApplVar.DeptNumber;
						ApplVar.DeptNumber = ReadPluDept(PluNumber);
						SaveDept = ApplVar.Dept;
						ReadDept();
				...
				if(type == 1){
					...
					Add(&ApplVar.NoVatRGItems.RGBuf[i].Qty, &MyRGRec.Qty);/* 合并单品或部类的数量 */
				}
				return 0;
			}
	}
	if (type == 3){
		if(!CheckNotZero(&TmpQty))
			return -1;
		t = 1;
		for (j = 0; j < 8; j++)
    	{
	    	if (Tax & t)
	    	{
				...
				memcpy(Qty.Value, ApplVar.Tax.Rate, sizeof(ApplVar.Tax.Rate));
				Amt = ZERO;
				Amt = ApplVar.NoVatRGItems.RGBuf[i].Amt;
				if (!TESTBIT(ApplVar.Tax.Options, BIT0)) /* add on ? */
				{
			        Qty.Sign = 0x04;
			        Multiply(&Amt, &Qty);
				...
				else
				{
					...
					sVal1 = ApplVar.TaxItem[ApplVar.AP.Tax.Number - 1]; //获取总的免税额
					sVal1.Sign = Amt.Sign;
					...
					sVal1.Sign = 0x0;
					if(CompareBCD(&Val1, &sVal1) > 0){ //1毛钱以内
						Amt = sVal2;
					}
					//Amt.Sign |= 0x80;
				}
				ApplVar.NetVatAmt = Amt;
	...
	if(type == 5)//jdb2018-08-09 没有记录返回-1
		return -1;
	
	ApplVar.NoVatRGItems.RGBuf[ApplVar.NoVatRGItems.RGNumber] = MyRGRec;
	ApplVar.NoVatRGItems.RGNumber++;
...
/* if ApplVar.BufCmd = 5 no vat*/ //jdb2018-04-08
...
void StoreInBuffer()
{
    ...
    BYTE i, savenum;
    short j;
	struct TRANSRECORD  TmpRGRec; //jdb2018-04-08
    ...
    if (ApplVar.BufCmd)
    {
		for(i = 0; i < ApplVar.RGItems.RGNumber; i++)
		{
			if (ApplVar.RGItems.RGBuf[i].Key.Code > DEPT && ApplVar.BufCmd == 5)  //jdb2018-04-08
			{
				TmpRGRec = ApplVar.RGItems.RGBuf[i];
				StoreInBuffer2(1, TmpRGRec);
				//ApplVar.RGItems.RGBuf[i].Key.Code = 0x0;
				continue;
			}
		    else if (ApplVar.RGItems.RGBuf[i].Key.Code == ApplVar.RGRec.Key.Code /* same ? */
				&& ApplVar.RGRec.Key.Code!=SUB)    /* ccr20130425+SUB,由于增加了把SUB操作存入RGBuf中,SUB不能合并 */
		    {
		    	if(ApplVar.RGRec.Key.Code > DEPT && ApplVar.RGRec.NoVat == 0x01){ //jdb2018-04-08
		    		//TmpRGRec = ApplVar.RGRec;
					//StoreInBuffer2(0, TmpRGRec);
					//return;
		    	}
				...
				if (ApplVar.RGRec.Key.Code > DEPT)  /* dept or plu ? */
				{
				    ...
				    if (ApplVar.BufCmd == 2)  /* correction */
				    {
						...
						TmpRGRec = ApplVar.RGItems.RGBuf[i]; //jdb2018-04-09
						TmpRGRec.Qty = TmpQty; //jdb2018-04-09
                    ...
                    {
                    	if(ApplVar.RGRec.NoVat == 0x01) //jdb2018-04-09
                    	{
                    		TmpRGRec = ApplVar.RGRec;
							StoreInBuffer2(0, TmpRGRec);
							ApplVar.RGRec.NoVat = 0x0;
                    	}
...
//						Multiply(&ApplVar.RGRec.DiscAmt,&ApplVar.RGRec.Qty);		/* cc 2007-02-13 */
	...
	if(ApplVar.BufCmd == 5) //jdb2018-04-08
	{
		ApplVar.BufCmd = 1;
		if(ApplVar.RGRec.Key.Code > DEPT){
			ApplVar.RGRec.NoVat = 0x01;
		}
		return;
	}
    if (ApplVar.BufCmd == 3)
    ...
    if (ApplVar.BufCmd == 2)       /* correction ?*/
    {
		...
		if (!CheckNotZero(&TmpQty)){
			...
			if(StoreInBuffer2(2, TmpRGRec) < 0){
				TmpRGRec = ApplVar.RGRec;
				if(StoreInBuffer2(3, TmpRGRec) < 0)
					ApplVar.ErrorNumber=ERROR_ID(CWXXI16);
			}
		}
		#endif
