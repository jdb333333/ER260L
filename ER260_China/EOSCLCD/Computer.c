...
BYTE SendRecord(BYTE *p ,BYTE length )
{
    ...
    BYTE *record;
    short sACK;
	unsigned char udata;//jdb2018-08-16 接收串口数据
    ...
    //ccr2016-04-18    for (retry=0;retry<5;retry++)
    {
...
#endif
        {
			...
			#else//jdb2018-08-16 直接读取串口数据
			for(i=3;i;i--){
				if(UARTGet(COMPUTER_1+1,&udata,5000)){
					if(udata&0xff != ACK && udata&0xff != NAK)
						continue;
				}
			}
			if(i == 0 || udata == NAK)
        ...
        *record = cpT;
		...
		//SendComm(COMPUTER_1, ACK);//jdb2018-08-16
		#else//jdb2018-08-16 直接读取串口数据
		for(i=3;i;i--){
			if(UARTGet(COMPUTER_1+1,&udata,5000)){
				if(udata&0xff != ACK && udata&0xff != NAK)
...
#endif
        {
            ...
            //ccr2016-04-20    	if (Appl_Port.Type=='1')//HOST mode
            {
                while (TRUE)
                {
					...
					//if(UARTGet(COMPUTER_1+1,&udata,100))//jdb2018-08-16 获取一个串口数据
                    {
                        FromUSB = 0;
                        bcc = i & 0xff;
						//bcc = udata & 0xff;//jdb2018-08-16 串口数据
						#if 0
						if(bcc != SOH)//jdb2018-08-16 测试用
							SendToHost(udata);
						#endif
                    }
#if defined(STM32F10X_HD_USBCOMM)
