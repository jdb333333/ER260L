...
void ProcessDept()
{
	...
	BCD sVal1, sVal2, sVal3; //jdb2018-04-10

    ApplVar.FCurr = 0;

	if (ApplVar.FVoid || ApplVar.FCorr) //jdb2018-05-02 为取消更正交易
	{
	}else{
		RESETBIT(ApplVar.FZV, BIT3);
	}
    ...
    {	//Multi Input of the same DEPT
		...
		{
			ApplVar.LastNetVatAmt = ZERO; //jdb2018-04-12 没按取消上一笔交易 为正常销售
...
//		    	ApplVar.FPrice = 0;
		...
		if (!ApplVar.MultiplyCount)
		    ApplVar.Qty = ONE;
		if(!ApplVar.FVoid){ //没按取消上一笔交易 为正常销售
			ApplVar.LastNetVatAmt = ZERO; //jdb2018-04-12
		}
		ApplVar.RepeatCount = 1;
	...
	if(ApplVar.FRefund == 1){ //jdb2018-04-12 退货免税标志
		ApplVar.FReVat = 1;
	}else{
		ApplVar.FReVat = 0;
	}
...
//ccr2016-03-04        if (SCREENLN-(TESTBIT(ApplVar.ScreenStat,STATEON))<CursY+3 || ApplVar.RepeatCount>1)//屏幕上空余行数<3
		...
		//SaveAmt = ApplVar.Amt; //jdb2018-04-10
		if(CheckNotZero(&ApplVar.NetVatAmt)){ //jdb2018-04-08
			Add(&ApplVar.Amt, &ApplVar.NetVatAmt);
		}
		if(ApplVar.FVoid && CheckNotZero(&ApplVar.LastNetVatAmt)){ //jdb2018-04-10 免税后，VOID操作
			sVal1 = ApplVar.TaxItem[ApplVar.AP.Tax.Number - 1]; //获取总的免税额
			sVal1.Sign = ApplVar.LastNetVatAmt.Sign;
					
			sVal2 = sVal1; //保存总免税额
			Subtract(&sVal1, &ApplVar.LastNetVatAmt);
					
			sVal1.Sign = 0x0;
			if(CompareBCD(&Val1, &sVal1) > 0){ //1毛钱以内
				ApplVar.LastNetVatAmt = sVal2;
			}
					
			Add(&ApplVar.Amt, &ApplVar.LastNetVatAmt);
		}
		...
		sVal3 = ApplVar.Cost;
		ApplVar.Cost = ZERO;//jdb2018-06-07 GRAND TOTAL 去掉COST
		AddGroupTotal();
		ApplVar.Cost = sVal3;
...
//				Collect_Data(PLULOG);
		...
		if(CheckNotZero(&ApplVar.NetVatAmt) || (ApplVar.FVoid && CheckNotZero(&ApplVar.LastNetVatAmt))){ //jdb2018-04-08
			CalculateTax(6);
		}else{
    ...
    if(ApplVar.FVoid && ApplVar.RepeatCount == 0){
    	ApplVar.LastNetVatAmt = ZERO; //jdb2018-04-10
    }
	//ApplVar.Amt = SaveAmt; //jdb2018-04-10
