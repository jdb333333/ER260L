...
void DispTAX(void);
void DispVAT(void);//jdb2018-07-19 免税显示函数
void DispZERO(void);//jdb2018-07-19 零税显示函数
...
CONST struct ECRLOGDES LogSize[LOGTYPEMAX]={
    ...
    {sizeof(struct CDC_VAT),         &DispVAT},         //jdb2018-07-19 免税项
    {sizeof(struct CDC_ZERO),         &DispZERO},         //jdb2018-07-19 零税项
#if defined(CASE_INDIA)//ccr2016-06-03>>>>>>>
...
short Collect_Data(BYTE cmd)
{
    ...
    case PLULOG:
        ...
        break;
	case VATLOG://jdb2018-07-19 免税流水
		if (TESTBIT(ApplVar.Amt.Sign,BIT7))
            ApplVar.FlowBuff.CDC_Vat.FunN |= 0x80;
		
		memcpy(ApplVar.FlowBuff.CDC_Vat.FAmt, ApplVar.Amt.Value, sizeof(ApplVar.FlowBuff.CDC_Vat.FAmt)); //净销售额
		memcpy(ApplVar.FlowBuff.CDC_Vat.FTax, ApplVar.Cost.Value, sizeof(ApplVar.FlowBuff.CDC_Vat.FTax)); //免税额
		Length = sizeof(ApplVar.FlowBuff.CDC_Vat);
		break;
	case ZEROLOG://jdb2018-07-19 零税流水
		if (TESTBIT(ApplVar.Amt.Sign,BIT7))
            ApplVar.FlowBuff.CDC_Zero.FunN |= 0x80;
		
		memcpy(ApplVar.FlowBuff.CDC_Zero.FAmt, ApplVar.Amt.Value, sizeof(ApplVar.FlowBuff.CDC_Zero.FAmt)); //净销售额
		memcpy(ApplVar.FlowBuff.CDC_Zero.FTax, ApplVar.Cost.Value, sizeof(ApplVar.FlowBuff.CDC_Zero.FTax)); //免税额
		Length = sizeof(ApplVar.FlowBuff.CDC_Zero);
		break;
...
void DispVAT()
{
	...
	sAmt = ZERO;
	memcpy(sAmt.Value,(char *)&ApplVar.FlowBuff.CDC_Vat.FAmt,sizeof(ApplVar.FlowBuff.CDC_Vat.FAmt));
	SETDECIMAL(sAmt.Sign, 2);

	memset(ProgLineMes, ' ', sizeof(ProgLineMes));
    strcpy(ProgLineMes,VATEXEMPTSALES);
	PrintAmt(ProgLineMes, &sAmt);
}
//jdb2018-07-19 显示零税
void DispZERO()
{
	...
	sAmt = ZERO;
	memcpy(sAmt.Value,(char *)&ApplVar.FlowBuff.CDC_Zero.FAmt,sizeof(ApplVar.FlowBuff.CDC_Zero.FAmt));
	SETDECIMAL(sAmt.Sign, 2);
...
void LoadReceipLog()
{
    ...
    while (Read_Flow((BYTE *)&ApplVar.FlowBuff)>0)
    {
        ...
        case VATLOG://jdb2018-07-31 增加免税流水打印
		case ZEROLOG://jdb2018-07-31 增加零税流水打印
        case SUBTOTALLOG:
