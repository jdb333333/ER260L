...
#endif
{
	...
	#ifndef ISP_NET
	#else
	//ApplVar.IP.IPAddress[0] = 192;
	//ApplVar.IP.IPAddress[1] = 168;
	//ApplVar.IP.IPAddress[2] = 99;
	//ApplVar.IP.IPAddress[3] = 222;
	Ethernet_Start(ApplVar.IP.IPAddress[0],ApplVar.IP.IPAddress[1],ApplVar.IP.IPAddress[2],ApplVar.IP.IPAddress[3]);//jdb2018-03-16
	#endif
...
#endif
		UART_Computer();//jdb2018-08-14 增加串口通讯
        //TCP_Computer(); //communicate with computer
...
BYTE TestChangePointMust()
{
    return((ApplVar.Key.Code != MULT &&
            ...
            ApplVar.Key.Code!=SCLOGIN && //jdb2018-03-08
            ApplVar.Key.Code!=PWDLOGIN && //jdb2018-03-08
			ApplVar.Key.Code!=VATEXEMPT && //jdb2018-03-20
...
void ProcessKey()
{
    ...
    if ((ApplVar.Key.Code < DISC || ApplVar.Key.Code > DISC + 100) &&
        (ApplVar.Key.Code < TEND || ApplVar.Key.Code > TEND + 100)){
			...
			if(ApplVar.Key.Code == SUB || ApplVar.Key.Code == SUB1){//jdb2018-04-03 reset
			}else{
				RESETBIT(ECRVIP.ECRVIPState, BIT4);
			}
        }
...
//cc 20070718>>>>>>>>>>>>>>
	...
	#if 0 //jdb2018-06-27 关闭浏览销售数据功能【开启固定折扣功能】
