...
//jdb2018-04-10
WORD ReadPluDept(WORD PluNumber)
{
	WORD SavePluNumber, PluDept;
	struct PLURECORD SavePlu;
	
	SavePluNumber = ApplVar.PluNumber;
	//memcpy(&SavePlu, &ApplVar.Plu, sizeof(SavePlu));
	SavePlu = ApplVar.Plu;
	...
	PluDept = ApplVar.Plu.Dept;

	ApplVar.PluNumber = SavePluNumber;
	//memcpy(&ApplVar.Plu, &SavePlu, sizeof(SavePlu));
	ApplVar.Plu = SavePlu;
...
//jdb2018-06-14
void DispNotFound(void)
{
	short pwdY;
	char pMessage[] = {NOTFOUND};
...
void CheckMixMatch(short bufchk)
{
    ...
    if (TESTBIT(ART_LEVEL, BIT2) && (ApplVar.AP.Plu.Cost || TESTBIT(ApplVar.MyFlags,PLUOFF)))// && !ApplVar.FPb) /* mix match active ? */
    {
        ...
        else if (TESTBIT(ApplVar.MyFlags,PLUDISC))//ccr 050613
        {
            ...
            ApplVar.DiscAmt = ZERO;
            memcpy(ApplVar.DiscAmt.Value, ApplVar.Plu.Price[1], ApplVar.AP.Plu.PriceSize);
						memcpy(ApplVar.Price.Value, ApplVar.Plu.Price[0], ApplVar.AP.Plu.PriceSize);//jdb2018-08-02 获取单价
        ...
        if (scmd != 0)
        {
            ...
            if (ApplVar.DiscNumber < ApplVar.AP.Disc.Number);
            {
				...
				if(TESTBIT(ApplVar.FZV, BIT5))
				{
					//jdb2018-07-03 不加入折扣报表
				}
				else
				{
                	AddDiscTotal();
