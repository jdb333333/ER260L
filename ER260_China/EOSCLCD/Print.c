...
void PrintHead1(short line)
{
    ...
    //ApplVar.PrintLayOut = 0 ;    ----0905 Zengy Logout
    {
		...
		#else //jdb2018-03-23 修改为40行
        for (; line < HeaderMax; line++)
        {
        	#if 0
        	if(line < 8){
            	if (ApplVar.TXT.Header[line][0])
            	...
            	if (ApplVar.AddHeader[line - 8][0])
                	PrintStr_Center(ApplVar.AddHeader[line - 8],DBLHIPRN & BIT3);
            	else
                	break;
			}
			#else //jdb2018-03-26
		...
		//jdb2018-07-26 add transaction title
		if(ApplVar.CentralLock == RG){
			
			if(ApplVar.FRefund)
			{
				PrintLine('*');
				PrintStr_Center(RETURNSLIP,BIT3);
				PrintLine('*');
				SETBIT(ApplVar.ECRRGFLAG, BIT0);
			}
			else
			{
				PrintLine('*');
				PrintStr_Center(TRTITELE,BIT3);
				PrintLine('*');
			}
		}
		#endif
...
void PrintRegiInfo(BYTE prnGrp)
{
	...
	BCD temp, temp1;//jdb2018-07-27 用于票据+1计算
	...
	//jdb2018-07-26 add receipt number prefix
	/*
    ...
    SysBuf[sP++]=' ';
	strcpy(SysBuf + sP, RECNUMPRE);
	SysBuf[strlen(SysBuf)] = ' ';
	//GetReceiptNumber(SysBuf +  PRTLEN -9);
	SysBuf[PRTLEN - 9] = '#';
	HEXtoASC(SysBuf +  PRTLEN - 8, ApplVar.ReceiptNumber, 4);
	flags = 1;
	PrintStr(SysBuf);
	*/
...
/*ApplVar.Clerk :********    Saler :********    */
        ...
        if (!TESTBIT(COMMA, BIT3))         
        {
            //GetReceiptNumber(SysBuf +  PRTLEN -7);
            if(ApplVar.CentralLock == XX || ApplVar.CentralLock == ZZ){//jdb2018-07-27    X Z报表加入打印票据单号区间
            	temp = ZERO;
				temp1 = ZERO;
            	memcpy(temp.Value, ApplVar.ZReceiptNumber, RECNUM);
				memcpy(temp1.Value, ApplVar.ReceiptNumber, RECNUM);
				if(CompareBCD(&temp, &temp1) < 0)
					Add(&temp, (BCD*)&ONE);
				memcpy(temp1.Value, ApplVar.ZReceiptNumber, RECNUM);//jdb2018-08-01 保存ZReceiptNumber
				memcpy(ApplVar.ZReceiptNumber, temp.Value, RECNUM);
			
            	SETBIT(ApplVar.ECRRGFLAG, BIT2);
				GetReceiptNumber(SysBuf +  PRTLEN -23);
				RESETBIT(ApplVar.ECRRGFLAG, BIT2);
				SysBuf[PRTLEN -12] = '-';
				GetReceiptNumber(SysBuf +  PRTLEN -11);

				#if 0
				if(ApplVar.CentralLock == XX)//jdb2018-08-01 X报表恢复ZReceiptNumber
					memcpy(ApplVar.ZReceiptNumber, temp1.Value, RECNUM);
				else//jdb2018-08-01 只有Z报表才重新计算ZReceiptNumber
					memcpy(ApplVar.ZReceiptNumber, ApplVar.ReceiptNumber, RECNUM);
				#else//jdb2018-08-14 只有Z报表PLU报表打印完后才重新计算ZReceiptNumber
					if(ApplVar.CentralLock == ZZ && ApplVar.ReportNumber == 2)
						memcpy(ApplVar.ZReceiptNumber, ApplVar.ReceiptNumber, RECNUM);
					else
						memcpy(ApplVar.ZReceiptNumber, temp1.Value, RECNUM);
				#endif
            ...
            else if(TESTBIT(ApplVar.ECRRGFLAG, BIT0)){ //jdb2018-07-26 退货
            	GetReceiptNumber(SysBuf +  PRTLEN -10);
				//RESETBIT(ApplVar.ECRRGFLAG, BIT0);
            }else
				GetReceiptNumber(SysBuf +  PRTLEN -11); //jdb2018-07-26 增加到8个数字+2(OR)
    ...
    if (ApplVar.FReceipt)    /* Receipt Used ? */
    {
        ...
        if (ApplVar.BufKp != 2)
        {
            if (ApplVar.TXT.Trailer[0][0])
            {
                	...
                	if(i < 6){
                    	if (ApplVar.TXT.Trailer[i][0])
                    	{
                        	PrintStr_Center(ApplVar.TXT.Trailer[i],DBLHIPRN & BIT4);
                       		...
                       		break;
                	}else if(i < TrailerMax){
                		#if 0
                	    if (ApplVar.AddTrailer[i - 6][0])
                    	{
                        	PrintStr_Center(ApplVar.AddTrailer[i - 6],DBLHIPRN & BIT4);
                    	}
                    	else
                       		break;
						#else //jdb2018-03-26
				...
				if(ApplVar.CentralLock == RG){
				//jdb2018-07-26 退货时增加打印
				if(TESTBIT(ApplVar.ECRRGFLAG, BIT0)){
					PrintStr_Center(RETFOOTER1,BIT4);
					PrintStr_Center(FOOTER2,BIT4);
					PrintStr_Center(FOOTER3,BIT4);
					PrintStr_Center(FOOTER4,BIT4);
					PrintStr_Center(FOOTER5,BIT4);
					//RESETBIT(ApplVar.ECRRGFLAG, BIT0);
				}
				else//jdb2018-08-01 每张票据尾都增加
				{
					PrintStr_Center(FOOTER1,BIT4);
					PrintStr_Center(FOOTER2,BIT4);
					PrintStr_Center(FOOTER3,BIT4);
				}
				}
