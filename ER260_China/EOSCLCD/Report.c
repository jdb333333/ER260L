...
void PrintCorrec(WORD number)
{
    ...
    ReadCorrec();
		...
		memset(SysBuf, ' ', sizeof(SysBuf));//jdb2018-08-01 不打印序号
...
void PrintDisc(WORD number)
{
	...
	/*
	if(ApplVar.DiscNumber == 8){
		memset(ApplVar.Disc.Name, 0, sizeof(ApplVar.Disc.Name));
		strcpy(ApplVar.Disc.Name, "SC DISCOUNT");
	}
	if(ApplVar.DiscNumber == 9){
		memset(ApplVar.Disc.Name, 0, sizeof(ApplVar.Disc.Name));
		strcpy(ApplVar.Disc.Name, "PWD DISCOUNT");
	}
	*/
	//PrintStr(ApplVar.Disc.Name);
	#endif
...
void PrintTax(WORD number)
{
    ...
    {
    	//jdb2018-03-21
    	if(ApplVar.TaxNumber + 1 == ApplVar.AP.Tax.Number){ //最后一个税为SC PWD减税额
    		#if 0
    		memset(SysBuf, 0, sizeof(SysBuf));
			strcpy(SysBuf, "VAT_SC/PWD");
			PrintStr(SysBuf);
			#else //jdb2018-03-29 打印空行
			PrintLine(' ');
			#endif
    	}else{
    		#if 0
...
void PrintReport(short RepFor,struct REPSIZE *S)
{
	...
	//if (ApplVar.Report.Type == REPORT_DEPARTMENT) //jdb2018-03-29 不打印部类
		//return;
    ...
    {
    	CheckPrinter(3);//jdb2018-06-20 等待打印机应答
		save = RamOffSet;
		if (ApplVar.RepComputer == 'C' && ApplVar.FReport == Z)
		{
            ...
            if (ApplVar.Report.Type!=0 || ApplVar.Report.Period!=0 || (TESTBIT(DBLHIPRN,BIT6) == 0)){ //jdb2018-04-02
				if(ApplVar.Report.Type != REPORT_GRANDTOTAL){//jdb2018-03-22 不清除Grand Total
					ResetReport();
				}else{
					ApplVar.BufCG = 1; //jdb2018-03-28
					//计算更新汇总数据
    				...
    				{
    					ApplVar.PointerType = 0; //jdb2018-04-03
						ApplVar.Size = ApplVar.AP.Sales.Size[ApplVar.PointerType];
						AddPointerTotal();
   			 		}
					ApplVar.BufCG = 0;
				}
            }else if(TESTBIT(DBLHIPRN,BIT6)){ //jdb2018-04-02 清除Grand Total
            	ResetReport();
            }
		...
		if (TESTBIT(ApplVar.Total.Qty.Sign, BIT2))     /* used ? */
		{
			...
			if(ApplVar.FReport == Z && ApplVar.Report.Type == REPORT_GRANDTOTAL && TESTBIT(DBLHIPRN,BIT6)==0 && ApplVar.Size.RetQty != 0 && ApplVar.FONP != 0x01){ //jdb2018-04-08
				ApplVar.BufCG = 1;
				//计算更新汇总数据
				RamOffSet = ApplVar.AP.StartAddress[AddrTotl];
            ...
            //ccr20140403 英文版的普通收款机需要保留Grand Total累计量，该计数器在每次Z-report后不清除
            //else if (ApplVar.Report.Type!=0 || ApplVar.Report.Period!=0 || (TESTBIT(DBLHIPRN,BIT6))){ //ccr2016-02-23
            else if(ApplVar.FReport == Z){ //jdb2018-04-12 其他报表清零
            	ResetReport();
            }
			#if 0
            if (ApplVar.Report.Type!=0 || ApplVar.Report.Period!=0 || (TESTBIT(DBLHIPRN,BIT6)==0)){ //jdb2018-04-02
				if(ApplVar.Report.Type != REPORT_GRANDTOTAL){//jdb2018-03-22 不清除Grand Total
					...
					#if 1
					if(ApplVar.FReport == Z && ApplVar.Size.RetQty != 0){
						ApplVar.BufCG = 1; //jdb2018-03-28
						//计算更新汇总数据
						RamOffSet = ApplVar.AP.StartAddress[AddrTotl];
						//for (ApplVar.PointerType = 0; ApplVar.PointerType < REPDEFMAX; ApplVar.PointerType++)	  /* add total sales */
	...
	if(ApplVar.Report.Type == REPORT_DISCOUNT && CheckNotZero(&ApplVar.DiscAmtTotal)){ //jdb2018-03-30 增加打印DISC总计
		PrintLine('*');
		PrintAmt(DISCOUNTTOTAL, &ApplVar.DiscAmtTotal);
		ApplVar.DiscAmtTotal = ZERO;
	}
...
void GetReport(BYTE xzNum)
{
    ...
    {//为在收款机上打印报表
		...
		PrintStr(SysBuf);
		if(repnum == 1 && ApplVar.FReport==Z){//jdb2018-07-27 Change Title Daily Report-Z to TERMINAL REPORT
			strcpy(SysBuf, TERMINALREPORT);
		}else{
		if (repnum == XZNUM)
    ...
    if (!SetUpReport())
    {
		...
		if (TESTBIT(ApplVar.AP.ReportList[repnum].Options, BIT2) && ApplVar.FReport == Z)
		{
			if ((ApplVar.Report.PointerType == 1) && TRAINING)
				...
				MemSet(ApplVar.ReturnNumber, RETNUM, 0);//jdb2018-07-26
			}
		}
