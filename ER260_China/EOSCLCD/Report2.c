...
void ReportEnd(BYTE AddZ)
{
	...
	//jdb2018-07-26 打印ENDING BALANCE
	//jdb2018-08-01 只有Z报表打印
	if(ApplVar.CentralLock == ZZ && TESTBIT(ApplVar.ECRRGFLAG, BIT1)){
		MemSet(SysBuf, PRTLEN, ' ');
		PrintAmt(TOTALACCSALES, &ApplVar.ENDBALANCE);
		PrintLine('-');
		RESETBIT(ApplVar.ECRRGFLAG, BIT1);
	}
    ...
    if (ApplVar.Report.Type < 12 && AddZ)
    {
		...
		if (ApplVar.FReport == Z && ApplVar.ReportNumber == 1)//jdb2018-07-27 只有Z日报表打印Z-COUNT
        {
    ...
    case MENUXREPORT: // 打印X报表
        ...
        do {
            ...
            {//打印X/Z报表
				...
				if(idx == 0x02){ //jdb2018-05-02 period report 不打印old grand total 和 new grand total
					ApplVar.FONP = 0x01;
    ...
    case MENUZREPORT:  //打印Z报表
        do {
            ...
            {//打印X/Z报表
                ...
                ApplVar.FReport = ApplVar.CentralLock;
				#if 0 //jdb2018-06-13 不需要周报表
				if(idx == 0x02){ //jdb2018-04-24 period report 不打印old grand total 和 new grand total
					ApplVar.FONP = 0x01;
					GetReport(ApplVar.ReportNumber);
					ApplVar.FONP = 0x00;
				}else
				#endif
				{
					if(ApplVar.ReportNumber != 4)//jdb2018-07-27 收款员日报表不打印
                		GetReport(ApplVar.ReportNumber);
				}
                ApplVar.FReport = 0;
            }
