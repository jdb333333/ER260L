		...
		if (Appl_EntryCounter)
		{
		    ...
		    {//所需付款金额处理完毕(输入的金额大于所需支付金额)
				...
				if (!TESTBIT(ApplVar.Tend.Options, BIT4))   /* overtender is tipp? */
				{
				    ...
				    PrintAmt(ApplVar.Tend.Name, &ApplVar.Entry);//jdb2018-06-20 collect 改为 cash
				}
				else//ccr20130417
...
//cc epos<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		{//所需付款金额支付完毕
			...
			//jdb2018-03-28 增加净销售额（减税后）
			{
			ApplVar.Cost = ApplVar.NetVatAmtTotal;
			ApplVar.NetVatAmtTotal = ZERO;
			ApplVar.VatAmtTotal = ZERO; //jdb2018-04-03
			ApplVar.FNetAmt = 1; //jdb2018-03-28
			}
			
		    ProcessBuffer();	/* reset Customer count and print kp */

			ApplVar.FReVat = 0; //jdb2018-04-12 按付款键后 退货免税标志置零
		    ApplVar.PbNumber = 0;
	...
	RESETBIT(ApplVar.ECRRGFLAG, BIT0);//jdb2018-07-26 退货标志置零
}
