...
//jdb2018-03-19
#define DISC_SCSUB_PERDEC	(DISC+9)      //为SC小计金额折扣
#define DISC_PWDSUB_PERDEC	(DISC+10)      //为SC小计金额折扣
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
//jdb2018-03-08
#define SCLOGIN      2095        // SC登录
#define PWDLOGIN      2096        // PWD登录
//jdb2018-03-19
#define VATEXEMPT	2097
//jdb2018-04-24 增加零税销售
#define ZERORATED	2098
...
=============================================================================================================*/
    ...
    #define cUP         cLAST	// 12
    #define cCHANGEASC 0x01//
    #else //jdb2018-04-17 WISE-158D
	#define cENTER     0x33
    #define cEXIT      0x1d
    #define cSELECT    0x28
    #define cDELETE    0x31
    #define cNEXT      0x11	//17         //right arrow
    #define cRIGHT     cNEXT
    #define cLAST      0x10	//16         //left arrow
    #define cLEFT      cLAST
    #define cSHIFT     0x01
    #define cCLEAR   	0x2f
    #define cDOWN      	cNEXT  // 11
    #define cUP         cLAST	// 12
    #define cCHANGEASC 0x0a//
	#endif
...
#define ID_DELETE   ID_RJFEED   //在编辑输入时,用作删除输入
    ...
    #define fDATE		0xff		//    feed paper anytime    */
    #else //jdb2018-04-17 WISE-158D
	#define fCLEAR		cCLEAR
	#define fRJFEED	    0x00
	#define fOCPRINTER	0x16
	#define fLOCK		0xff
	#define fREPORT	    0xff
	#define fPRGTYPE	cSELECT
	#define fPRGNUM	    0xff
	#define fPRGENTER	cENTER
	#define fPRGOPT	    0xff         	//     Programming option invert
    #define fSYSREP	    0xff         //     system report trigger
    #define fHARDTEST	0xff         //     Machine test key
    #define fDUMP		0xff         //     Dump Program key
...
#define tQTY    0
#define tAMT    4
#define tCOST   4 //jdb2018-04-27 3-->4
#endif//ccr2015-08-20<<<<<<<<<<<<
//#define taxNumber			3 	    /* number of tax itemizers */
#define taxNumber			8 	    //jdb2018-03-24
...
#define offNumber			(50)//jdb2018-06-27	    /* number of OFF */
#endif
...
#define pluInvSize			5	//jdb2018-06-22 库存最大值 4 【999999】
...
#else
//jdb2018-03-20
#define discNumber			10	//增加SC PWD 两类DISC
#endif
...
#define toCC     2
#define toQTY    4
#define toAMT    5 //new grand total
#define toRET    5 //old grand total
#define toDISC   4
#define toCOST   5 //net amount
#endif
#endif//ccr2015-08-20<<<<<<<<<<<<
...
typedef enum {
	#if 0 //jdb2018-06-13 不需要当前收款员报表
    REPID_CLERKDAILY,           //收款员日报
    REPID_CLERKPERIOD,          //收款员周报
    #endif
	REPID_CLERKDAILY,//jdb2018-07-27 需要收款员日报表
    REPID_DAILYREPORT,         //销售日报
    #if 0 //jdb2018-06-13 不需要周报表
    REPID_PERIODREPORT,        //销售周报
    #endif
    REPID_PLUREPORT,           //商品报表
    #if 0 //jdb2018-06-13 不需要桌账报表
...
typedef enum {
    ...
    #endif
	ItemXZ_CLERKDAILY, //jdb2018-07-27需要收款员日报表
...
struct TRANSRECORD
{
	...
	BYTE NoVat; //jdb2018-04-08 免税=0x01
	//BCD OffAmt; //jdb2018-08-02 记录特殊出价减去的金额
};
...
struct CLERKRECORD
{
    ...
    BYTE Passwd[3];
	BYTE Options2;//jdb2018-05-05 增加标识
				//BIT0: 1 限制增加PLU
};
...
#else //jdb2018-03-26 增加到40行
struct APPTEXT
{
    BYTE    Header[HeaderMax][PRTLEN+1];    /* max width printer 32 char */
    BYTE    ReportType[17][REPWIDTH];
    BYTE    SlipHeader[6][SHEADWIDTH+1];
    BYTE    Trailer[TrailerMax][PRTLEN+1];
};
#endif
...
struct APPLICATION
{
    ...
    BYTE SHOPNAME[32]; //jdb2018-07-12 增加商铺名称
...
/************************ Working Memory lay-out ************************/
    ...
    BYTE
        ...
        ReceiptNumber[RECNUM+TRAINNUM];       /* >>>>>>>>>!!!  receipt number BCD max 999999 */
	BYTE	
		ReturnNumber[RETNUM],//jdb2018-07-26 增加退货单号计数
		ZReceiptNumber[RECNUM];//jdb2018-07-26-7记录Z报表打印时的票据单号
#if defined(CASE_QRCODE)
    BYTE
...
//        FSuspend,
        ...
        SubDiscFlag,	/*cc 2006-10-11 for SubDisc
						...
						*/
		SCPWDDiscFlag,  //jdb2018-08-02 BIT0  标记为SC PWD 折扣
	...
	struct RECEPTITEMS  NoVatRGItems;        //jdb2018-04-08 免税销售项目
    ...
    BCD
		...
		//jdb2018-03-09
        LessVatAmt,				//记录单笔减税销售总额
        LessDiscAmt,			//SC PWD小计后折扣额 -20%
        LessVatTotalAmt,
        LastVatExempt, //jdb2018-06-14 保存上次的vat exempt 值
        VatAmtTotal,
        NetVatAmtTotal,
        DiscAmtTotal,
        NetVatAmt,
		...
		//jdb2018-05-03 增加零税销售总额
		ZEROLessVatAmt,
		ZEROAMOUNT,

		ENDBALANCE,//jdb2018-07-26 增加ending balance value
		REFECRNUM,//jdb2018-07-26 REF#退货票据号
...
//jdb2018-03-22
	BYTE	AddTrailer[34][PRTLEN+1];
	BYTE	AddHeader[32][PRTLEN+1];
	BYTE 	BufCG; //jdb2018-03-28 清GRAND标志
	BYTE 	FNetAmt; //jdb2018-03-28 保存净销售额标志
	BYTE 	FReVat; //jdb2018-04-12 退货免税标志
	BYTE 	FZV; //jdb2018-04-24 BIT3增加零销售和免税标志 BIT1 免税 BIT2 零销售 BIT5 特殊出价免税
	BYTE 	FONP; //jdb2018-04-24 增加打印OLD GRAND TOTAL 和 NEW GRAND TOTAL标志 0x0 打印 0x1 不打印
	WORD    SPluNumber; //jdb2018-06-20 保存上传的plu数
	BYTE 	FREP; //jdb2018-07-03 报表标志 BIT1 GRANDTOTAL
										   //BIT2 DEPART
										   //BIT3 TENDER
										   //BIT4 DRAWER
										   //BIT5 DISCOUNT
										   //BIT6 TAX
	BYTE 	ECRRGFLAG;//jdb2018-07-26 BIT0 退货标志       BIT1打印ENDING BALANCE BIT2打印上一次Z报表票据号
	BYTE  	INPUTFUN; //jdb2018-07-12 输入功能 
							//BIT0 输入字母       
							//BIT1 输入无符号整型
							//BIT2 输入浮点数
	unsigned long LNumberEntry;//jdb2018-07-27 无符号长整型
